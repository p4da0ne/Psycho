...
CalculatingProblemManager::CalculatingProblemManager(QWidget *parent) //int in_id_object
    : QWidget(parent),
	UI (new Ui::calculatingproblemmanager_form)
{
    UI->setupUi(this);
...
void CalculatingProblemManager::otchet()
{
	otch = new QDialog;

	QPushButton *Save_as_pdf = new QPushButton("Save as pdf");
...
void CalculatingProblemManager::save_pdf()
{
...
int CalculatingProblemManager::calc_info_for_region(QString id_region)
{
...
int CalculatingProblemManager::calc_mps_for_ls(float n)
{
	if(n > 0 && n < 0.3)
		return 1;
	if(n >= 0.3 && n < 0.6)
...
CalculatingProblemManager::~CalculatingProblemManager()
{
	delete UI;
}

//======= Формирование списка таблиц управления объектами =======
void CalculatingProblemManager::init_object_tree()
{
	QFont font;
	font.setBold(true);
	QString sSql;
...
//================== контекстное меню =========================================
void CalculatingProblemManager::customMenuView(const QPoint & pos)
{
 QModelIndex index = UI->columnView->currentIndex();
	...
	if (id.type() == QVariant::String){ 
		...
		if(list.value(0)=="region"){
			...
			QAction *act = new QAction(QString("Оценка морально-психологической обстановки: %1").arg(list.value(3)),this);
			...
			QAction *act=new QAction(QString("Оценка морально-психологического состояния %1: %2").arg(list.value(3)).arg(list.value(2)),this);
...
void CalculatingProblemManager::customMenuTree(const QPoint & pos)
{
...
void CalculatingProblemManager::show_objects(const QModelIndex &index)
{
	...
	if (id.type() == QVariant::String) { 
	QString user_data=id.toString();
	 QProgressDialog progress("Формирование информации о регионах", "Отмена", 0, 2, this);
...
void CalculatingProblemManager::child_region_objects(QStandardItem *parent_item,int id_parent_region)
{
	//select all Region 
	
	QSqlQuery query;
	int row=0;
	query.exec(QString("SELECT id_region, name_region, parent_region FROM region WHERE parent_region=%1").arg(id_parent_region));
...
void CalculatingProblemManager::add_region_components(QStandardItem *parent_item,int id_parent_region,int start_row){
//СМИ ====================================================================================================
	QSqlQuery query;
	...
	{//	ветка СМИ
		...
		while (query.next())
		{
			...
			{
				set_child_item(query_smi.value(1).toString(),QString("dsmi_%1").arg(id_smi_region) + "_" + QString::number(id_smi),item,row_sw);
			}
			row_sw++;
		}
...
//Воинские формирования =============================================================================================
	query.exec(QString("SELECT id_region, id_ls,name_ls FROM ls WHERE id_region=%1").arg(id_parent_region));
	int row_vf=0;
	int b; 
	...
	item=set_child_item("ВОИНСКИЕ ФОРМИРОВАНИЯ ["  + QString::number(b) + "]",QString("ls_%1").arg(id_parent_region),parent_item,row,"./icons/user_config_org.png");
	row++;
	if (query.size() != 0)
	{//Ветка воинские формирования
		...
		while (query.next())
		{
			...
			child_ls_objects( set_child_item(query.value(2).toString(),QString("chls_%1_%2_%3").arg(id_ls).arg(mps_for_ls).arg(calc_mps.isEnemy(id_ls)),item,row_vf,iconsList.at(calc_mps_for_ls(mps_for_ls))),
								id_ls);
			row_vf++;
		}
	}
	query.clear();
//Организации =============================================================================================
	query.exec(QString("SELECT id_region, id_groups FROM groups WHERE id_region=%1").arg(id_parent_region));
	int row_gr=0;
...
void CalculatingProblemManager::column_item_clicked ( const QModelIndex &index){
...
void CalculatingProblemManager::child_ls_objects(QStandardItem *parent_item,int parent_ls)
{
	QSqlQuery query;
	int row=0;
	query.exec(QString("SELECT id_ls, name_ls, parent_ls FROM ls WHERE parent_ls=%1").arg(parent_ls));
		...
		while (query.next()){
			...
			child_ls_objects(set_child_item(query.value(1).toString(),
				"lss_" + QString::number(id_object) + "_" + QString::number(parent_ls) + "_" + QString::number(mps_for_ls) + "_" + QString("%1").arg(calc_mps.isEnemy(id_object)), 
												parent_item,
												row,iconsList.at(calc_mps_for_ls(mps_for_ls))),id_object);
			row++;
			}
	}
...
void CalculatingProblemManager::add_ls_components(QStandardItem *parent_item,int id_parent_region,int start_row){
//========================= Национальность =========================================================
	QFont font;
	font.setBold(true);
	QSqlQuery query;
	...
	QStandardItem *item=set_child_item("Национальный состав",QString("nations_%1").arg(id_parent_region),parent_item,row,font);
	row++;
	int id_nations;
	int id_ls_nations;
	if (query.size() != 0)
...
QStandardItem * CalculatingProblemManager::set_child_item(QString item_text,QString user_data,QStandardItem *parent_item,int row)
	{
		QStandardItem *item = new QStandardItem(item_text);
		item->setData(user_data,Qt::UserRole);
		parent_item->setChild(row,item);
		return item;
	}
QStandardItem * CalculatingProblemManager::set_child_item(QString item_text,QString user_data,QStandardItem *parent_item,int row,QFont font)
	{
		QStandardItem *item = new QStandardItem(item_text);
		item->setFont(font);
		item->setData(user_data,Qt::UserRole);
...
QStandardItem * CalculatingProblemManager::set_child_item(QString item_text,QString user_data,QStandardItem *parent_item,int row,QString icon_path)	{
		QStandardItem *item = new QStandardItem(QIcon(icon_path),item_text);
		item->setData(user_data,Qt::UserRole);
		parent_item->setChild(row,item);
		return item;
	}
QStandardItem * CalculatingProblemManager::set_child_item(QString item_text,QString user_data,QStandardItem *parent_item,int row,QString icon_path,QFont font)	{
		QStandardItem *item = new QStandardItem(QIcon(icon_path),item_text);
		item->setFont(font);
		item->setData(user_data,Qt::UserRole);
		parent_item->setChild(row,item);
...
void CalculatingProblemManager::region_click(int id_region){
	...
	//model_kvo->setRelation(2,QSqlRelation("country", "id_country", "name_country"));
	//model_kvo->setRelation(2,QSqlRelation("type_kvo", "id_type_kvo", "name_type_kvo"));
	...
	model_region->setHeaderData(13, Qt::Horizontal, "Уровень бедности(спф1)");model_region->setHeaderData(14, Qt::Horizontal, "Pспф1");
	model_region->setHeaderData(15, Qt::Horizontal, "Уровень цен(спф2)");model_region->setHeaderData(16, Qt::Horizontal, "Pспф2");
	model_region->setHeaderData(17, Qt::Horizontal, "Уровень образования(спф3)");model_region->setHeaderData(18, Qt::Horizontal, "Pспф3");
	model_region->setHeaderData(19, Qt::Horizontal, "Степень доверия населения(спф4)");model_region->setHeaderData(20, Qt::Horizontal, "Pспф4");
	model_region->setHeaderData(21, Qt::Horizontal, "Степень поддержки населением(спф5)");	model_region->setHeaderData(22, Qt::Horizontal, "Pспф5");
	model_region->setHeaderData(23, Qt::Horizontal, "Поддержка гос.структур(спф6)");	model_region->setHeaderData(24, Qt::Horizontal, "Pспф6");
	model_region->setHeaderData(25, Qt::Horizontal, "Поддержка организациями ВС(спф7)");model_region->setHeaderData(26, Qt::Horizontal, "Pспф7");
	model_region->setHeaderData(27, Qt::Horizontal, "Влияние оппозиционных организаций(спф8)");model_region->setHeaderData(28, Qt::Horizontal, "Pспф8");
	model_region->setHeaderData(29, Qt::Horizontal, "Поддержка авторитетных деятелей(спф9)");model_region->setHeaderData(30, Qt::Horizontal, "Pспф9");
	model_region->setHeaderData(31, Qt::Horizontal, "Уровень безработицы(спф10)");model_region->setHeaderData(32, Qt::Horizontal, "Pспф10");
	model_region->setHeaderData(33, Qt::Horizontal, "Миграция населения (спф11)");model_region->setHeaderData(34, Qt::Horizontal, "Pспф11");
	model_region->setHeaderData(35, Qt::Horizontal, "Демография(спф12)");model_region->setHeaderData(36, Qt::Horizontal, "Pспф12");
	model_region->setHeaderData(37, Qt::Horizontal, "Уровень информатизации(спф13)");model_region->setHeaderData(38, Qt::Horizontal, "Pспф13");
	model_region->setHeaderData(39, Qt::Horizontal, "Направленность информации(спф14)");model_region->setHeaderData(40, Qt::Horizontal, "Pспф14");
	model_region->setHeaderData(41, Qt::Horizontal, "Защищенность инфроструктуры(спф15)");model_region->setHeaderData(42, Qt::Horizontal, "Pспф15");
	model_region->setHeaderData(43, Qt::Horizontal, "Неправительственные организации(спф16)");model_region->setHeaderData(44, Qt::Horizontal, "Pспф16");
	model_region->setHeaderData(45, Qt::Horizontal, "Уровень патриотизма(спф17)");model_region->setHeaderData(46, Qt::Horizontal, "Pспф17");
	model_region->setHeaderData(47, Qt::Horizontal, "Уровень преступности(кф1)");model_region->setHeaderData(48, Qt::Horizontal, "Pкф1");
	model_region->setHeaderData(49, Qt::Horizontal, "Корупция(кф2)");model_region->setHeaderData(50, Qt::Horizontal, "Pкф2");
	model_region->setHeaderData(51, Qt::Horizontal, "Теневой сектор(кф3)");model_region->setHeaderData(52, Qt::Horizontal, "Pкф3");
	model_region->setHeaderData(53, Qt::Horizontal, "Влияние некоренного населения(кф4)");model_region->setHeaderData(54, Qt::Horizontal, "Pкф4");
	model_region->setHeaderData(55, Qt::Horizontal, "Экстремизм,НВФ(кф5)");model_region->setHeaderData(56, Qt::Horizontal, "Pкф5");
	model_region->setHeaderData(57, Qt::Horizontal, "Исправительные учереждения(кф6)");model_region->setHeaderData(58, Qt::Horizontal, "Pкф6");
	model_region->setHeaderData(59, Qt::Horizontal, "Протестная активность(кф7)");model_region->setHeaderData(60, Qt::Horizontal, "Pкф7");
	model_region->setHeaderData(61, Qt::Horizontal, "Организованная преступность(кф8)");model_region->setHeaderData(62, Qt::Horizontal, "Pкф8");
	model_region->setHeaderData(63, Qt::Horizontal, "Оружие,наркотрафик(кф9)");model_region->setHeaderData(64, Qt::Horizontal, "Pкф9");
	model_region->setHeaderData(65, Qt::Horizontal, "Межнациональные конфликты(кф10)");model_region->setHeaderData(66, Qt::Horizontal, "Pкф10");
	model_region->setHeaderData(67, Qt::Horizontal, "Поддержка религиозных объединений(рф1)");model_region->setHeaderData(68, Qt::Horizontal, "Pрф1");
	model_region->setHeaderData(69, Qt::Horizontal, "Культовые сооружения(рф2)");model_region->setHeaderData(70, Qt::Horizontal, "Pрф2");
	model_region->setHeaderData(71, Qt::Horizontal, "Авторитет религиозных лидеров(рф3)");model_region->setHeaderData(72, Qt::Horizontal, "Pрф3");
	model_region->setHeaderData(73, Qt::Horizontal, "Поддержка религиозных лидеров(рф4)");model_region->setHeaderData(74, Qt::Horizontal, "Pрф4");
	model_region->setHeaderData(75, Qt::Horizontal, "Религиозность противника(рф5)");model_region->setHeaderData(76, Qt::Horizontal, "Pрф5");
	model_region->setHeaderData(77, Qt::Horizontal, "Религиозность своих войск(рф6)");model_region->setHeaderData(78, Qt::Horizontal, "Pрф6");
	model_region->setHeaderData(79, Qt::Horizontal, "Код значка региона");
	...
	UI->property_object->setColumnWidth(77,210);UI->property_object->setColumnWidth(78,50);UI->property_object->setColumnWidth(79,140);
	UI->property_object->setShowGrid(true);

	UI->property_object->setItemDelegate(new QSqlRelationalDelegate(UI->property_object));
//=============================================================================================
}
void CalculatingProblemManager::smi_click(int id_smi){
...
//	UI->property_object->setAlternatingRowColors(true);
	...
	UI->property_object->setShowGrid(true);
	QSqlRelationalDelegate *delegat=new QSqlRelationalDelegate(UI->property_object);
	UI->property_object->setItemDelegate(delegat);
}

void CalculatingProblemManager::ls_click (int id_ls)
{
QSqlRelationalTableModel *model_region = new QSqlRelationalTableModel(this);
...
UI->property_object->sortByColumn(2,Qt::AscendingOrder);
	...
	UI->property_object->setColumnWidth(2,200);UI->property_object->setColumnWidth(3,200);//UI->property_object->setColumnWidth(3,200);
	UI->property_object->setColumnWidth(4,200);UI->property_object->setColumnWidth(6,200);UI->property_object->setColumnWidth(7,200);
...
void CalculatingProblemManager::gr_click (int id_gr)
{
QSqlRelationalTableModel *model_region = new QSqlRelationalTableModel(this);
...
void CalculatingProblemManager::national_show_form(int id_region)
{
