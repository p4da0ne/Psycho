...
{	
    ...
    if (!((QString(value2)=="mpo_pso")||(QString(value2)=="special_conditions")))
    {
        ...
        {
            emit signal_for_change_scale(pe, info, num_obj, id_obj,  region); //сигнал испускается при клике на любое "пустое" место карты
        }
    }
    else
    {
        emit signal_for_change_scale(pe, info, num_obj, id_obj,   region); //сигнал испускается при клике на "mpo_pso" или на "special_conditions"
    }
...
void MapScroll::freeObject(HOBJ hobj)
{
    map->mapFreeObject(hobj);
}

void MapScroll::deleteObject(HOBJ hobj)
{
