...
TextPrinter::TextPrinter(QObject *parent)
    : QObject(parent), parent_(0),
      ...
      leftmargin_(10.0), rightmargin_(10.0), topmargin_(5.0),
      bottommargin_(3.0), spacing_(0.0),
      headersize_(0.0), headerrule_(0.0), headertext_(QString()),
      footersize_(0.0), footerrule_(0.0), footertext_(QString()),
      dateformat_()
{
    ...
    printer_->setPageSize(QPageSize::A4);
...
{
    printer_->setPageOrientation(orientation);
}
...
{
    if ((margin > 0) && (margin < printer_->paperRect(QPrinter::Point).width() / 2)) {
        leftmargin_ = margin;
    } else {
        leftmargin_ = 0;
    }
...
{
    if ((margin > 0) && (margin < printer_->paperRect(QPrinter::Point).height() / 4)) {
        topmargin_ = margin;
    } else {
        topmargin_ = 0;
    }
...
{
    if ((margin > 0)
        && (margin < printer_->paperRect(QPrinter::Point).height() / 2)
        && (margin < printer_->paperRect(QPrinter::Point).width() / 2)) {
...
{
    if ((spacing > 0) && (spacing <= printer_->paperRect(QPrinter::Point).height() / 8)) {
        spacing_ = spacing;
    } else {
        spacing_ = 0;
    }
...
{
    if ((size > 0) && (size <= printer_->paperRect(QPrinter::Point).height() / 8)) {
        headersize_ = size;
    } else {
        headersize_ = 0;
    }
...
QRectF TextPrinter::paperRect(QPaintDevice *device)
{
    ...
    QRectF rect = printer_->paperRect(QPrinter::Point);
    // adjust for DPI
							...
							int save)
{
  ...
  //      exportname.append(".pdf");
    ...
    printer_->setOutputFormat(QPrinter::PdfFormat);
    printer_->setOutputFileName(exportname);
	//printer_->setFromTo(fromPage,toPage);
