...
void ReportView::preview_dialog(QString html)
{
    ...
    QPushButton *printButton = new QPushButton("ѕечать");
...
void ReportView::print_formul(QString html)
{
    ...
    textprinter_->print(report_window1, tr("ѕечать документа"));
}
...
void ReportView::print_pdf_formul(QString html)
{
    ...
    QString filename;
        //fd = new QFileDialog;
        //filename = fd -> getSaveFileName(this, tr("—охранить"),"c:/test/test.pdf", tr("PDF (*.pdf)"));
    ...
    textprinter_->setOrientation(QPrinter::Portrait);
    textprinter_->exportPdf(report_window1,"—охранить",filename);
}
